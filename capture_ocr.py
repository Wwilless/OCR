...
_cap_instance: "cv2.VideoCapture | None" = None
...
def get_pipeline() -> "OCRPipeline":
    ...
    if _pipeline_instance is None:
        ...
        _pipeline_instance.preload()
    return _pipeline_instance


def _get_cap() -> "cv2.VideoCapture":
    global _cap_instance
    if _cap_instance is None or not _cap_instance.isOpened():
        _cap_instance = cv2.VideoCapture(load_config()["camera_index"], cv2.CAP_DSHOW)
        _cap_instance.set(cv2.CAP_PROP_FRAME_WIDTH,  645)
        _cap_instance.set(cv2.CAP_PROP_FRAME_HEIGHT, 560)
        _cap_instance.set(cv2.CAP_PROP_BUFFERSIZE,   1)
    return _cap_instance


def scan_once_result() -> dict:
    cap = _get_cap()
    if not cap.isOpened():
        return {"status": -2, "message": "找不到攝影機"}
    cap.grab()
    ret, frame = cap.retrieve()
    if not ret:
        return {"status": -2, "message": "無法讀取畫面"}

    if get_red_dark_ratio(frame) >= CHECK_RED_RATIO:
        return {"status": -1, "message": "無瓶子"}

    if not check_bottle_orientation(frame):
        return {"status": 0, "message": "培養瓶裝反了"}

    rx, ry, rw, rh = ROI
    labels = get_pipeline().scan(frame, (rx, ry, rw, rh))
    results = {}
    for label in labels[:len(ROW_LABELS)]:
        if ":" in label:
...
def main():
    ...
    cam_idx = load_config()["camera_index"]
    ...
    last_displayed_bottle = False
    last_red_check        = 0.0
    bottle_trigger_at     = 0.0
    ...
    while True:
        ...
        if time.time() - last_red_check >= CHECK_RED_INTERVAL:
            ...
            has_bottle = dark_ratio < CHECK_RED_RATIO

            if has_bottle != last_displayed_bottle:
                print("Bottle detected" if has_bottle else "無瓶子", flush=True)
                last_displayed_bottle = has_bottle
                if has_bottle:
                    bottle_trigger_at = time.time()
